...
-- All residents are active (archived = 0), age column removed (calculated dynamically)
--
...
-- Only admin account (password: admin123)
--
...
1. ARCHIVE FUNCTIONALITY:
   - Added 'archived' column to all main tables (residents, certificates, blotter_records)
   - Default value: 0 (active), 1 (archived)
   - Archived records are hidden from main listings but preserved in database
   - Added indexes for better performance

2. DYNAMIC AGE CALCULATION:
   - Removed 'age' column from residents table
   - Age is now calculated dynamically using: TIMESTAMPDIFF(YEAR, birthdate, CURDATE())
   - Ensures always accurate, real-time age calculation

3. ADMIN-ONLY SYSTEM:
   - Removed user registration functionality
   - Removed test user account
   - Only admin account remains (username: admin, password: admin123)
   - Modified users table to only allow 'admin' role

4. PROPER RESIDENT LINKAGE:
   - Blotter records properly linked to residents via foreign keys
   - Complainant and respondent names follow "Surname, Firstname Middlename" format
   - Certificate names also follow consistent naming format

5. DATABASE RELATIONSHIPS:
   - Foreign key constraints ensure data integrity
   - ON DELETE SET NULL prevents data loss when residents are archived
   - Proper indexing for optimal performance

MIGRATION INSTRUCTIONS:
1. Backup your existing database
2. Run this SQL file to create fresh database with all updates
3. Or use migrate_all.php to add archive columns to existing database
4. Update any custom queries to exclude archived records (WHERE archived = 0)
